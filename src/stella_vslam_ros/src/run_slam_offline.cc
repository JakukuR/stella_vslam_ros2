...
// Helper: decompress a CompressedImage into a sensor_msgs::msg::Image
static std::shared_ptr<sensor_msgs::msg::Image> decompress_image(
    const std::shared_ptr<sensor_msgs::msg::CompressedImage>& compressed_msg) {
    // Decode JPEG/PNG compressed data using OpenCV
    cv::Mat decoded = cv::imdecode(cv::Mat(compressed_msg->data), cv::IMREAD_UNCHANGED);
    if (decoded.empty()) {
        return nullptr;
    }

    // Parse original encoding from CompressedImage format field
    // format is like "rgb8; jpeg compressed" or "bgr8; png compressed"
    std::string original_encoding;
    const auto& fmt = compressed_msg->format;
    auto sep = fmt.find(';');
    if (sep != std::string::npos) {
        original_encoding = fmt.substr(0, sep);
        // trim whitespace
        while (!original_encoding.empty() && original_encoding.back() == ' ')
            original_encoding.pop_back();
    }

    auto img_msg = std::make_shared<sensor_msgs::msg::Image>();
    img_msg->header = compressed_msg->header;
    img_msg->height = decoded.rows;
    img_msg->width = decoded.cols;
    img_msg->step = decoded.step;

    if (decoded.type() == CV_8UC3) {
        // imdecode always returns BGR channel order.
        // If original was rgb8, we need to swap channels back to RGB.
    ...
    } else if (decoded.type() == CV_16UC1) {
        img_msg->encoding = "16UC1";
    } else {
        img_msg->encoding = original_encoding.empty() ? "bgr8" : original_encoding;
    }

    img_msg->data.assign(decoded.data, decoded.data + decoded.total() * decoded.elemSize());
    return img_msg;
}

static bool is_compressed_topic(const std::string& topic) {
    // Check if the topic name ends with "/compressed"
...
void tracking(const std::shared_ptr<stella_vslam_ros::system>& slam_ros,
    ...
    if (slam_ros->slam_->get_camera()->setup_type_ == stella_vslam::camera::setup_type_t::Monocular) {
        ...
        const bool color_is_compressed = is_compressed_topic(color_topic);
        const bool depth_is_compressed = is_compressed_topic(depth_topic);
        rclcpp::Serialization<sensor_msgs::msg::CompressedImage> compressed_serialization;
        if (color_is_compressed) {
            RCLCPP_INFO(slam_ros->node_->get_logger(), "Color topic is compressed, will decompress");
        }
        if (depth_is_compressed) {
            RCLCPP_INFO(slam_ros->node_->get_logger(), "Depth topic is compressed, will decompress");
        }
        ...
        while (rclcpp::ok()) {
            while (reader.has_next() && que.size() < 2) {
                ...
                if (bag_message->topic_name == color_topic) {
                    rclcpp::SerializedMessage serialized_msg(*bag_message->serialized_data);
                    if (color_is_compressed) {
                        auto cmsg = std::make_shared<sensor_msgs::msg::CompressedImage>();
                        compressed_serialization.deserialize_message(&serialized_msg, cmsg.get());
                        auto img = decompress_image(cmsg);
                        if (img) que.push(img);
                    } else {
                        auto msg = std::make_shared<sensor_msgs::msg::Image>();
                        serialization.deserialize_message(&serialized_msg, msg.get());
            ...
            const auto tp_1 = std::chrono::steady_clock::now();
            if (que_depth.size() > 0) {
                rgbd->depth_callback(que_depth.front());
                que_depth.pop();
            }
            rgbd->color_callback(que.front());
            const auto tp_2 = std::chrono::steady_clock::now();
            track_time = std::chrono::duration_cast<std::chrono::duration<double>>(tp_2 - tp_1).count();
