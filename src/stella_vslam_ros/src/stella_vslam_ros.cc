...
rgbd::rgbd(const std::shared_ptr<stella_vslam::system>& slam,
    ...
    color_sub_ = node_->create_subscription<sensor_msgs::msg::Image>(
        ...
        [this](sensor_msgs::msg::Image::ConstSharedPtr msg) { color_callback(msg); });
    depth_sub_ = node_->create_subscription<sensor_msgs::msg::Image>(
        "camera/depth/image_raw", qos,
        [this](sensor_msgs::msg::Image::ConstSharedPtr msg) { depth_callback(msg); });
}

void rgbd::depth_callback(const sensor_msgs::msg::Image::ConstSharedPtr& depth) {
    std::lock_guard<std::mutex> lock(depth_mtx_);
    latest_depth_ = depth;
}

void rgbd::color_callback(const sensor_msgs::msg::Image::ConstSharedPtr& color) {
    ...
    // Resize color image to match depth image if sizes differ
    if (colorcv.size() != depthcv.size()) {
        cv::Mat resized_color;
        cv::resize(colorcv, resized_color, depthcv.size());
        colorcv = resized_color;
    }
